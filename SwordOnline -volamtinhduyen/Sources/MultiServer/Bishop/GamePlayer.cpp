...
bool CGamePlayer::FreezeMoney( const char *pAccountName, int nExtPoint, int nChangeExtPoint)
{
	if ( !pAccountName || !pAccountName[0] )
	{
		return false;
...
UINT CGamePlayer::VerifyAccount()
{
	...
	if ( pRetBuffer )
	{
		...
		used = (m_sAccountName.length() < used) ? m_sAccountName.length() : (used - 1);
...
UINT CGamePlayer::SelAddDelRole()
{
	...
	if ( pRetBuffer )
	{
		...
		nAccountLen = (m_sAccountName.length() < nAccountLen) ? m_sAccountName.length() : (nAccountLen - 1);
	...
	if ( pRetBuffer )
	{
		...
		if ( _CreateNewPlayer_ToDBRole( ( const char * )szRoleName, pNPC->m_btRoleNo, pNPC->m_btSeries, pNPC->m_NativePlaceId ) )
		{
	...
	if ( pRetBuffer )
	{
		...
		for (int i=0;i<m_sPlayerCount;i++)
		{
			if (m_sPlayerName[i].compare(pDSPC->szRoleName) == 0 && m_sAccountName.compare(pDSPC->szAccountName) == 0)
			{
...
bool CGamePlayer::_QueryRoleInfo_ToDBRole( const char *pRoleName, const char * pAccountName )
{
