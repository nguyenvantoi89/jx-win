...
void KUiOptions::LoadSetting(bool bReload, bool bUpdateOption)
{
	...
	if (bUpdateOption && g_pCoreShell)
	{
		...
		g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_PLAYER, bOptionsEnable[OPTION_H_PLAYER]);
		g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_SKILL, bOptionsEnable[OPTION_H_SKILL]);
		g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_NPC, bOptionsEnable[OPTION_H_NPC]);
	}
...
void KUiOptions::ToggleOption(int nIndex)
{
	...
	case OPTION_H_PLAYER:	//透视模式
		if (g_pCoreShell)
			g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_PLAYER, bEnable);
		break;	
	case OPTION_H_SKILL:	//透视模式
		if (g_pCoreShell)
			g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_SKILL, bEnable);
		break;	
	case OPTION_H_NPC:	//透视模式
		if (g_pCoreShell)
			g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_HIDE_NPC, bEnable);			
		break;
	}
	UpdateAllStatusImg();
}
