...
{
	return sprintf(pString, "%02d:%02d | %02d-%02d-%d",
			ptm->tm_hour,
			ptm->tm_min,
			ptm->tm_mday,
			ptm->tm_mon + 1,
...
void KUiPlayerBar::OnSend(BOOL bDirectSend)
{
	...
	{
		char szWarning[] = "Vui lßng kh«ng sö dông tõ ng÷ kh«ng thÝch hîp trong giao tiÕp!";
...
void KUiPlayerBar::OnSendChannelMessage(DWORD nChannelID, const char* Buffer, int nLen)				//·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
	if (nChannelID != -1 && Buffer && nLen > 0)
	{
		...
		{
			char szWarning[64];
			sprintf(szWarning, "B¹n cÇn nghØ ng¬i sau [%d] gi©y míi cã thÓ tiÕp tôc truyÒn tin", (nLeft + 999 ) / 1000);
	...
	while(KUiMsgCentrePad::GetChannelData(dwID, cost, Buffer, nLen))
	{
		...
		if (!bSend)
		{
			if (nRet == 2 && nUseLevel > 0)
				sprintf(szSystem, "§¼ng cÊp cña b¹n kh«ng ®ñ [%d], kh«ng thÓ sö dông [Thiªn lý truyÒn ©m c«ng] ®Ó chuyÓn ph¸t tin tøc", nUseLevel);

			if (nRet == 1 && nUseMoney > 0)
				sprintf(szSystem, "Money cña b¹n kh«ng ®ñ [%d] ®Ó sö dông Thiªn lý truyÒn ©m c«ng", nUseMoney);

			if (nRet == 3 && nUseMana > 0)
				sprintf(szSystem, "Néi lùc cña b¹n kh«ng ®ñ [%d] ®Ó sö dông Thiªn lý truyÒn ©m c«ng", nUseMana);
		}
		else
		{
			if (nUseMoney > 0)
				sprintf(szSystem, "B¹n ®· sö dông [Thiªn lý truyÒn ©m c«ng] chuyÓn ph¸t lµm tiªu hao [%d] money", nUseMoney);
			if (nUseMana > 0)
				sprintf(szSystem, "B¹n ®· sö dông [Thiªn lý truyÒn ©m c«ng] chuyÓn ph¸t lµm tiªu hao [%d] néi lùc", nUseMana);
		}
