...
BOOL KRegion::LoadObject(int nSubWorld, int nX, int nY, char *lpszPath)
{
	...
	return TRUE;
#endif
/*	char	szPath[FILE_NAME_LENGTH], szFile[FILE_NAME_LENGTH];   // fix loi 1 so map goc lag
...
int KRegion::FindObject(int nMapX, int nMapY, bool bAutoFind)
{
	...
	while(pNode)
	{
		if (Object[pNode->m_nIndex].m_nMapX == nMapX && Object[pNode->m_nIndex].m_nMapY == nMapY)
		{
			if (bAutoFind)
			{
				if (Object[pNode->m_nIndex].m_nKind == Obj_Kind_Money || Object[pNode->m_nIndex].m_nKind == Obj_Kind_Item || Object[pNode->m_nIndex].m_nKind == Obj_Kind_Prop)
					return pNode->m_nIndex;
			}
			else
				return pNode->m_nIndex;
