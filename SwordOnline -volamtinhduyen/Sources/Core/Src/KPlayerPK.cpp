...
*/	
	...
	else if (m_nNormalPKFlag == 1)
	{
		...
		else if (Npc[Player[m_nPlayerIndex].m_nIndex].m_Level >= 10) // Level Do Sat
		{
			m_nNormalPKFlag = 2;
		}
	}
...
BOOL	KPlayerPK::EnmityPKOpen(int nAim)
{
	...
	// aaa
	if (m_bBlock && Npc[Player[m_nPlayerIndex].m_nIndex].m_SubWorldIndex != g_SubWorldSet.SearchWorld(33) && Npc[Player[m_nPlayerIndex].m_nIndex].m_SubWorldIndex != g_SubWorldSet.SearchWorld(37))
		return FALSE;
