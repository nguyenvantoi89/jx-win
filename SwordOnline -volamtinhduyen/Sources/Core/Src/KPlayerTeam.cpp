...
void	KPlayerTeam::ReceiveInvite(TEAM_INVITE_ADD_SYNC *pInvite)
{
	...
	nIdx = pInvite->m_nIdx;
	if (m_bAutoRefuseInviteFlag && Player[CLIENT_PLAYER_INDEX].m_bActiveAutoParty)
	{
		ReplyInvite(nIdx, 0);
	}
    else if (m_bAutoAccecpt && Player[CLIENT_PLAYER_INDEX].m_bActiveAutoParty && m_nFlag)  // fix loi vao pt 
        return;
...
BOOL	KTeam::SetTeamClose(BOOL bNotify)
{
	...
	if (bNotify)
		Player[m_nCaptain].SendSystemMessage(MESSAGE_TEAM_ANNOUCE_HEAD, MSG_TEAM_CLOSE); // thong bao loi paty 
