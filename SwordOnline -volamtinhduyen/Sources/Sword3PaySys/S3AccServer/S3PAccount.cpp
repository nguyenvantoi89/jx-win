...
//		{
			...
			if (pConn->Do(strSQL))
			{
				...
				if (nExtPoint > 0)
				{
					sprintf(strSQL, "update Account_Habitus set nExtPoint = %d where (cAccName = '%s')",nExtPoint,strAccName);
					pConn->Do(strSQL);
					nExtPoint = 0;
				}
			}
										...
										long& liNumLogin)
{
	...
	char strSQL[MAX_PATH];
	sprintf(strSQL, "select datediff(second, getdate(), dEndDate), iLeftSecond, nExtPoint, iNumLogin from View_AccountMoney where (cAccname = '%s')", strAccName);
	S3PResultVBC* pResult = NULL;
	if (pConn->QuerySql(strSQL, &pResult))
	{
		if (pResult->num_rows() <= 0)
